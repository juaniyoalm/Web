...
class ValidadorRegistro {
    ...
    private function validar_nombre($conexion, $nombre) {
        ...
        if (strlen($nombre) > 24) {
            return "El nombre no debe ocupar más de 24 caracteres";
        }

        if (RepositorioUsuario :: nombre_existe($conexion, $nombre)) {
            return "Este nombre de usuario ya esta en uso, prueba otro nombre.";
        }
        return "";
    }
    ...
    private function validar_email($conexion, $email) {
        ...
        if (RepositorioUsuario :: email_existe($conexion, $email)) {
            return "Este email ya esta en uso, prueba otro email o <a href='#'>intente recuperar su contraseña</a>.";
        }
